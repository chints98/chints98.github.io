{
	...
	const TiltObj = function(el, options) {
		...
		this.DOM = {};
		this.DOM.img = this.el.querySelector('.u-max-full-width');
		this.DOM.title = this.el.querySelector('.project-name');
		this._initEvents();
	};
    ...
    DOM.contenEtlems = Array.from(document.querySelectorAll('.row'));
    console.log(typeof(DOM.contenEtlems))

    console.log(DOM.contenEtlems.length)
	DOM.contentLinks = Array.from(document.querySelectorAll('.menu__item-name'));
	DOM.footer = document.querySelector('.footer-card');
	const contentElemsTotal = 8;
	const shapes = [
		{
